...
from app.agentic.teammate.context_builder import TeammateContextBuilder
...
class TeammateResponseService:
    ...
    def __init__(
        ...
        **_kwargs: Any,  # 兼容旧调用方传入的 flash_cpu= 等已废弃参数
    ) -> None:
        ...
        # 上下文构建器（提示词加载 + 可见性过滤 + 格式化）
        from app.world.npc.visibility import TeammateVisibilityManager
        self._ctx = TeammateContextBuilder(
            visibility_manager=TeammateVisibilityManager(),
            decision_prompt_path=self.decision_prompt_path,
            response_prompt_path=self.response_prompt_path,
            agentic_system_prompt_path=self.agentic_system_prompt_path,
        )

        # 默认模型配置
        self.default_model_config = TeammateModelConfig()

    # 上下文构建方法已迁移到 TeammateContextBuilder (self._ctx)
    ...
    async def process_round(
        ...
        active_members, world_id, gm_narration_full, is_private, private_target = (
            self._ctx.resolve_round_basics(
                party=party,
        ...
        # 3. 每个队友独立决策是否回复
        members_for_decision = self._ctx.members_for_decision(
            active_members=active_members,
            is_private=is_private,
            private_target=private_target,
            private_only=False,
    ...
    async def _decide_single_response(
        ...
        """决定单个队友是否回复"""
        # 准备上下文
        filtered_context = self._ctx.build_filtered_context(
            member=member,
            player_input=player_input,
            gm_response=gm_response,
            context=context,
        ...
        effective_player_input = self._ctx.get_effective_player_input(
        ...
        # 加载并填充提示词
        prompt_template = self._ctx.load_prompt(self.decision_prompt_path)
        if not prompt_template:
            # 无提示词，使用简单规则
            return self._simple_decision(member, effective_player_input, gm_response, context)

        location = self._ctx.get_location_context(context)
    ...
    async def process_round_stream(
        ...
        for member in active_members:
            ...
            try:
                ...
                if result.response:
                    for piece in self._ctx.chunk_text(result.response):
                        yield {
                            "type": "teammate_chunk",
                            "character_id": member.character_id,
                            "text": piece,
    ...
    # _get_agentic_system_prompt / _build_response_prompt 已迁移到 self._ctx
    ...
    async def _run_agentic_generation_payload(
        ...
        *,
        ...
        result = await executor.run(
            ctx=ctx,
            system_prompt=self._ctx.get_agentic_system_prompt(),
            user_prompt=prompt,
    ...
    async def _generate_response_payload(
        ...
        *,
        ...
        except (LLMServiceError, RuntimeError, KeyError) as exc:
            logger.warning(
    ...
    async def _generate_single_response_core(
        ...
        *,
        ...
        prompt = self._ctx.build_response_prompt(
            member=member,
            player_input=player_input,
            gm_response=gm_response,
            context=context,
