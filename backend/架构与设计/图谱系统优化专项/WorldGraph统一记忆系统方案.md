...
| 文件 | 操作 |
|------|------|
| `app/services/memory_graph.py` | 删除 |
| `app/services/graph_store.py` | 保留（仍用于首次会话知识加载 + 离线工具），但移除会话内调用 |
| `app/models/graph_scope.py` | 保留（知识加载仍需要），标记为初始化专用 |
| `app/services/flash_service.py` | 移除图相关方法（ingest_event/recall） |
| `app/services/instance_manager.py` | 移除 `_memory_graph` 字段 |
| `app/runtime/area_runtime.py` | 移除 `area_graph` 字段 |
| 所有 `from app.services.memory_graph import` | 清理 |
| 相关测试 | 适配或重写 |

**验收**：`MemoryGraph` 零引用（grep 确认），所有测试通过。
...
**M3（Phase 5+6 完成）**：MemoryGraph 删除，WorldGraph 是唯一图容器
