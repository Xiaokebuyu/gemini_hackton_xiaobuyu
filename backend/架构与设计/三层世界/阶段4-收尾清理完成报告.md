...
**测试结果（2026-02-21 复核）**：24 failed / 784 passed / 1 skipped
**非环境基线（排除 `fastapi_to_mcp` 与 `mcp_connection`）**：774 passed / 35 deselected
**说明**：当前失败集中在旧 E2E 断言（world_id 与已删除端点）和 MCP 服务环境依赖（9101/9102）。
...
```
全量：24 failed / 784 passed / 1 skipped
非环境：774 passed / 35 deselected
```

所有非环境测试全绿。架构收尾项已完成，剩余主要是测试口径与环境依赖收口。
