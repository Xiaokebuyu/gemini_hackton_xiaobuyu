...
创建时间：2026-02-26
状态：设计中
前置文档：`AI-Osiris设计规范.md`（P30 反应式姊妹组件），`编排层设计规范.md`（P35 Hook 集成），`内容层设计规范.md`（QuestRegistry 里程碑数据），`状态层设计规范.md`（NarrativePlanSlice + QuestSlice），`NPC与队友子系统设计规范.md`（PasserbyPool 投递 + NPC Directive），`叙事层设计规范.md`（NPC Agent 消费 Directive）
...
> **模块化设计**：`plant_environmental` 委托给独立模块 **DynamicSubAreaManager**（详见 `编排层设计规范.md` §9.5）。NarrativePlanner 只描述"要什么样的子区域"，创建和生命周期管理由 DynamicSubAreaManager 负责。这与 `spawn_quest_npc` 调用 PasserbyService 是同一模式。
>
...
> - `remove_sub_areas`: 移除临时子区域（调用 DynamicSubAreaManager.expire()，详见 `编排层设计规范.md` §9.5）
>
...
NarrativePlannerHook.execute()
    ...
    │
    ├─ 4. 指令执行（逐条验证合法性后执行）
    │     create_quest        → 写入 NarrativePlanSlice.active_quests + QuestSlice（DISCOVERED）
    ...
    │     plant_environmental → DynamicSubAreaManager.create()（编排层 §9.5）→ 写入 ❸ AreaSlice.temporary_sub_areas
...
| PasserbyService | **复用**：`spawn_quest_npc` 底层调用路人生成系统（`NPC与队友子系统设计规范.md` §九） |
| InstanceManager | **复用**：`direct_npc` 写入 NPC 工作记忆（`NPC与队友子系统设计规范.md` §八） |
| DynamicSubAreaManager | **复用**：`plant_environmental` 委托给动态子区域服务（`编排层设计规范.md` §9.5） |
...
| Q6 | 环境线索如何实现？ | **临时子区域**：`plant_environmental` 生成可探索的动态子地点，委托独立模块 DynamicSubAreaManager（编排层 §9.5）。不做 GM 独占叙述——多主体协作 |
| Q7 | 动态子区域归谁管？ | **编排层独立模块**：DynamicSubAreaManager 可被 NarrativePlanner / AI Osiris / EventEngine 等多个 Hook 调用，不绑定单一消费者 |
...
| 2026-02-26 | 补全 5 种缺失指令详细格式（§3.6-§3.10）：`plant_environmental` 设计为临时子区域生成（委托 DynamicSubAreaManager 独立模块，编排层 §9.5）；`publish_bulletin` 公告板可交互实体 + 驻留 NPC 通知；`escalate` 纯 Command 数值操作；`adjust_pacing` 自我调节；`retire_quest` 级联清理。更新执行流程（§4.3）和 NarrativePlanSlice（§7.1）。确立多主体协作叙述原则（非 GM 独占） |
