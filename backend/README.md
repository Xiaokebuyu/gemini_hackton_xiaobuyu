...
采用 **Agentic v3 架构**——模型在单次 Agentic 会话中自主调用 MCP 工具（导航、对话、战斗、图谱操作等），同时生成叙述，辅以工具执行强制校验和自动修复机制，兼顾自主性与可控性。
...
入口：`AdminCoordinator.process_player_input_v3()`
...
1. **Agentic 架构**：模型在单次会话中自主调用 MCP 工具并生成叙述，辅以工具执行强制校验与自动修复，兼顾自主性与可控性
