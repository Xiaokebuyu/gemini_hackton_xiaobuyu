...
class InstanceManager:
    ...
    def _make_key(self, world_id: str, npc_id: str, session_id: str = "") -> str:
        """生成实例键（含 session_id 隔离跨会话上下文污染）"""
        if session_id:
    ...
    async def get_or_create(
        ...
        Args:
            ...
            session_id: 会话 ID（用于跨会话隔离）

        Returns:
            NPCInstance 实例
        """
        key = self._make_key(world_id, npc_id, session_id)
    ...
    def get(self, world_id: str, npc_id: str, session_id: str = "") -> Optional[NPCInstance]:
        """
        获取实例（不创建）
    ...
    def has(self, world_id: str, npc_id: str, session_id: str = "") -> bool:
        """检查实例是否存在"""
        key = self._make_key(world_id, npc_id, session_id)
        return key in self._instances

    async def remove(self, world_id: str, npc_id: str, session_id: str = "", persist: bool = True) -> bool:
        """
        移除实例
    ...
    async def flush_all(self, world_graph: Any = None) -> int:
        """关机前 flush 所有实例的 ContextWindow 到图谱。"""
        flushed = 0
        for key, instance in list(self._instances.items()):
            if instance.context_window.message_count > 0:
                try:
                    await self._graphize_messages(
                        instance=instance,
                        current_scene=None,
                        game_day=1,
                        force_all=True,
