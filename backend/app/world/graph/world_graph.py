...
class WorldGraph:
    ...
    def mark_behavior_fired(self, node_id: str, behavior_id: str) -> None:
        """标记 behavior 已触发（once=True 用），自动标脏。"""
        node = self.get_node(node_id)
        if node:
            node.mark_behavior_fired(behavior_id)
            self._dirty_nodes.add(node_id)

    def set_behavior_cooldown(self, node_id: str, behavior_id: str, ticks: int) -> None:
        """设置 behavior 冷却计数，自动标脏。"""
        node = self.get_node(node_id)
        if node:
            node.set_behavior_cooldown(behavior_id, ticks)
            self._dirty_nodes.add(node_id)

    def tick_cooldowns(self, node_id: str) -> None:
        """递减节点所有 behavior 冷却计数，自动标脏。"""
        node = self.get_node(node_id)
        if node:
            node.tick_cooldowns()
            self._dirty_nodes.add(node_id)
