...
from app.exceptions import WorldGraphError
...
class SessionSnapshot(Protocol):
    """GraphBuilder 所需的最小会话接口，打断 builder ↔ session_runtime 循环依赖。"""
    ...
    _player_character: Any
from app.world.player.constants import default_npc_state, default_player_state
...
from app.world.graph.event_behaviors import _event_to_behaviors  # noqa: F401
...
class GraphBuilder:
    ...
    def build(
        ...
        Args:
            ...
            session: 满足 SessionSnapshot 协议的会话对象（队伍数据）。
...
def _build_chapters(wg: WorldGraph, world: WorldInstance) -> None:
    ...
    for ch_id, ch_data in world.chapter_registry.items():
        ...
        try:
            ...
            raise WorldGraphError(f"章节 '{ch_id}' 解析失败: {exc}") from exc
    ...
    for ch_id, ch_data in world.chapter_registry.items():
        try:
            ...
            raise WorldGraphError(f"GATE 阶段章节 '{ch_id}' 解析失败: {exc}") from exc
...
def _build_events(wg: WorldGraph, world: WorldInstance) -> None:
    ...
    for ch_id, ch_data in world.chapter_registry.items():
        try:
            ...
            raise WorldGraphError(f"事件解析失败(章节 '{ch_id}'): {exc}") from exc
        ...
        if default_map:
            if default_sub:
                ...
                if wg.has_node(host_id):
                    ...
                    wg.merge_state(char_id, {"current_location": default_map})
                    logger.debug(
    ...
    for area_id, area_def in world.area_registry.items():
        for sub_loc in area_def.sub_locations:
            ...
            for char_id in sub_loc.resident_npcs:
                ...
                wg.merge_state(char_id, {"current_location": loc_node_id})
...
def _build_party(wg: WorldGraph, session: SessionSnapshot) -> None:
...
def _build_player_node(wg: WorldGraph, session: SessionSnapshot) -> None:
    ...
    from app.world.player.node_view import translate_character_to_node
