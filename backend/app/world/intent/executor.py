...
def update_hosts_edges(wg, old_area_id: Optional[str], new_area_id: str) -> None:
    ...
    from app.world.graph.models import WorldEdgeType
...
class IntentExecutor:
    ...
    async def execute_move(self, destination_id: str, destination_name: str = "") -> EngineResult:
        ...
        if wg and wg.has_node(destination_id):
            try:
                ...
                logger.warning("[IntentExecutor] MOVE merge_state/visit_count failed for %s: %s", destination_id, exc)
            ...
            if is_area_change:
                ...
                try:
                    ...
                    logger.warning("[IntentExecutor] HOSTS edge sync failed %s→%s: %s", current_area_id, destination_id, exc)
    ...
    async def execute_examine(self, target_id: str, target_name: str = "") -> EngineResult:
        ...
        if wg:
            try:
                ...
                logger.warning("[IntentExecutor] EXAMINE get_node(%s) failed: %s", target_id, exc)
                node = None
            if node is not None:
    ...
    async def execute_use_item(self, item_id: str, item_name: str = "") -> EngineResult:
        ...
        from app.world.combat.rules import ITEM_EFFECTS
        ...
        heal_amount = 0
        try:
            from app.world.combat.dice import DiceRoller
            heal_amount, _ = DiceRoller.roll(heal_expr)
            heal_amount = max(0, int(heal_amount))
        except (ValueError, TypeError, ImportError) as exc:
            logger.warning("[IntentExecutor] USE_ITEM dice roll failed for '%s', fallback to 8: %s", heal_expr, exc)
            heal_amount = 8

        # 3) 先扣物品，成功后才加血（避免扣物品失败→免费回血）
        consumed = False
        if hasattr(player, "remove_item"):
            try:
                consumed = bool(player.remove_item(resolved_item_id, 1))
            except (KeyError, ValueError, AttributeError) as exc:
                logger.warning("[IntentExecutor] USE_ITEM remove_item(%s) failed: %s", resolved_item_id, exc)
                consumed = False
        if not consumed:
        ...
        if hasattr(self.session, "mark_player_dirty"):
            try:
                ...
                logger.warning("[IntentExecutor] USE_ITEM mark_player_dirty failed: %s", exc)
