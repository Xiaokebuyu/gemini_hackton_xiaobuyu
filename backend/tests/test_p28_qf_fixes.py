...
# ── QF-1: _private_chat_result_to_sse scene_change transition ─────────────────


class TestPrivateChatSceneTransition:
    def test_transition_fade_emitted_on_first_entry(self) -> None:
        """scene_change includes transition:'fade' when entering private scene for first time."""
        ...
        events = _private_chat_result_to_sse(result, current_location="pub_main_room")
        scene_events = [e for e in events if e.event_type == "scene_change"]
        assert len(scene_events) == 1
        assert scene_events[0].payload.get("transition") == "fade", (
    ...
    def test_no_scene_change_when_result_has_no_scene_id(self) -> None:
        """No scene_change event emitted when result.scene_id is None."""
        from app.agent_orchestration import _private_chat_result_to_sse
        from app.game_core.orchestration.private_chat import PrivateChatResult
    ...
    def test_transition_with_no_current_location(self) -> None:
        """scene_change includes transition:'fade' when current_location is None."""
        from app.agent_orchestration import _private_chat_result_to_sse
        from app.game_core.orchestration.private_chat import PrivateChatResult
        ...
        events = _private_chat_result_to_sse(result, current_location=None)
        scene_events = [e for e in events if e.event_type == "scene_change"]
        assert len(scene_events) == 1
        assert scene_events[0].payload.get("transition") == "fade"
