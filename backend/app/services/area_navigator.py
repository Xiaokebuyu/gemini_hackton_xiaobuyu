...
class MapConnection:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "MapConnection":
        ...
        try:
            ...
            connection_type = ConnectionType.TRAVEL
        return cls(
...
class AreaNavigator:
    ...
    def _load_maps_from_firestore(self) -> None:
        ...
        if not maps_data:
            raise ValueError(
                f"世界 '{self.world_id}' 未初始化地图数据（Firestore: worlds/{self.world_id}/maps）"
            )
