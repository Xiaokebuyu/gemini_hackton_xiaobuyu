...
# Empty frozenset means full access (GM).
_ROLE_ALLOWED_SLICES: dict[str, frozenset[str]] = {
    "gm": frozenset(),
    "npc": frozenset({"scene", "relations", "player", "time", "area"}),
    "teammate": frozenset({"scene", "relations", "player", "time", "area", "party"}),
}
...
class RoleStateProxy:
    ...
    def __init__(self, state: StateContainer, role: str) -> None:
        ...
        object.__setattr__(self, "_full_access", len(allowed) == 0)
