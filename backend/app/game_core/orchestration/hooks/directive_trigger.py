...
whether the target NPC is reachable, and fires an ``npc_wants_to_chat`` SSE
event so the frontend can surface the conversation invitation.  At most one
NPC invitation is emitted per tick to avoid overwhelming the player.
...
class DirectiveTriggerHook(NoOpSettlementHook):
    ...
    currently reachable, rolls a probability gate and — on success — emits an
    ``npc_wants_to_chat`` SSE event identical in shape to the one emitted by
    ``PrivateChatTriggerHook``, but with ``reason="directive"``.
    ...
    async def execute(self, context: SettlementContext) -> HookResult:
        ...
        for directive_entry in pending_directives:
            ...
            # Set cooldown flag (same pattern as PrivateChatTriggerHook)
            if has_flags:
            ...
            logger.debug(
                "DirectiveTriggerHook: %s wants to chat (directive priority=%s)",
                npc_id, priority,
            )
