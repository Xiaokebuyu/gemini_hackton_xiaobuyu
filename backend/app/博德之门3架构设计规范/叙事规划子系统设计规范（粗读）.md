...
NarrativePlannerHook.execute()
    ...
    │     escalate            → planner_escalate
    │     adjust_pacing       → planner_set_pacing_frozen
    │     retire_quest        → planner_retire_quest + 级联清理（despawn NPC / 移除公告 / 移除子区域）
    │
...
class NarrativePlannerHook(SettlementHook):
    ...
    async def execute(self, context: SettlementContext) -> HookResult:
        ...
        # planner 输出 story_facts → planner_add_story_facts + WorldKnowledgeGraph.inject_story_facts()
